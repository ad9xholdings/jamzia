/* ═══════════════════════════════════════════════════════════
   JamCompliance — Regulatory compliance
   Built by Collective General Technologies, LLC
   ═══════════════════════════════════════════════════════════ */

import { useState } from 'react';
import { ShieldCheck, Plus, Search, ChevronRight } from 'lucide-react';;
...
const INITIAL_ITEMS: Item[] = [
  { id: '1', name: 'JamCompliance Item Alpha', status: 'active', updated: '2 min ago' },
  { id: '2', name: 'JamCompliance Item Beta', status: 'pending', updated: '1 hr ago' },
  { id: '3', name: 'JamCompliance Item Gamma', status: 'active', updated: '3 hr ago' },
  { id: '4', name: 'JamCompliance Item Delta', status: 'archived', updated: '1 day ago' },
];

export default function JamCompliance() {
    ...
    <div className="min-h-screen bg-black text-white">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-lg bg-[#7096D1]/10 flex items-center justify-center">
                <ShieldCheck size={20} className="text-[#7096D1]" />
              </div>
              <div>
                <h1 className="text-lg font-bold">JamCompliance</h1>
                <p className="text-[10px] text-[#6B7280]">Regulatory compliance · Collective General Technologies, LLC</p>
              </div>
            </div>
            <button className="flex items-center gap-1.5 px-3 py-1.5 bg-[#7096D1] hover:bg-[#7096D1]/80 text-black rounded-lg text-xs font-bold transition-colors">
              <Plus size={12} /> New
            </button>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: 'Total', value: items.length.toString(), color: '#7096D1' },
        ...
        <div className="flex items-center gap-2">
          {(['all', 'active', 'pending'] as const).map(f => (
            <button key={f} onClick={() => setFilter(f)}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium capitalize transition-colors ${filter === f ? 'bg-[#7096D1] text-black' : 'bg-[#0A0A0A] text-[#6B7280] border border-[#1F1F1F]'}`}>
              {f}
            </button>
          ))}
        </div>
