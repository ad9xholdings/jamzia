/* ═══════════════════════════════════════════════════════════
   JamLocal — Local government services
   Built by Collective General Technologies, LLC
   ═══════════════════════════════════════════════════════════ */

import { useState } from 'react';
import { Landmark, Plus, Search, ChevronRight } from 'lucide-react';;
...
const INITIAL_ITEMS: Item[] = [
  { id: '1', name: 'JamLocal Item Alpha', status: 'active', updated: '2 min ago' },
  { id: '2', name: 'JamLocal Item Beta', status: 'pending', updated: '1 hr ago' },
  { id: '3', name: 'JamLocal Item Gamma', status: 'active', updated: '3 hr ago' },
  { id: '4', name: 'JamLocal Item Delta', status: 'archived', updated: '1 day ago' },
];

export default function JamLocal() {
    ...
    <div className="min-h-screen bg-black text-white">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-lg bg-[#22c55e]/10 flex items-center justify-center">
                <Landmark size={20} className="text-[#22c55e]" />
              </div>
              <div>
                <h1 className="text-lg font-bold">JamLocal</h1>
                <p className="text-[10px] text-[#6B7280]">Local government services · Collective General Technologies, LLC</p>
              </div>
            </div>
            <button className="flex items-center gap-1.5 px-3 py-1.5 bg-[#22c55e] hover:bg-[#22c55e]/80 text-black rounded-lg text-xs font-bold transition-colors">
              <Plus size={12} /> New
            </button>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: 'Total', value: items.length.toString(), color: '#22c55e' },
        ...
        <div className="flex items-center gap-2">
          {(['all', 'active', 'pending'] as const).map(f => (
            <button key={f} onClick={() => setFilter(f)}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium capitalize transition-colors ${filter === f ? 'bg-[#22c55e] text-black' : 'bg-[#0A0A0A] text-[#6B7280] border border-[#1F1F1F]'}`}>
              {f}
            </button>
          ))}
        </div>
